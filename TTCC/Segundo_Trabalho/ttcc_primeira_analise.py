...
for _, _, files in walk('./dados/Anonimizados/TTCC/'):
    for file in files:
        merge_tables = pd.merge(read_base(f'./dados/Anonimizados/TTCC/{file}'),alunos_base)
        base_data.append(
            decribe_data(
                merge_tables,
                './dados/Tabs/ttcc/',
...
save_table(tab1, name="Geral", path="./dados/Tabs/ttcc/")
