...
for _, _, files in walk('./dados/Anonimizados/MD/'):
    for file in files:
        merge_tables = pd.merge(read_base(f'./dados/Anonimizados/MD/{file}'),alunos_base)
        base_data.append(
            decribe_data(
                merge_tables,
                './dados/Tabs/md/',
...
save_table(tab1, name="Geral", path="./dados/Tabs/md/")
